    ...
    async run(client, message) {
        ...
        if (!message.guild.me.permissions.has("READ_MESSAGE_HISTORY")) {
            message.channel.send({
                content: "Please enable the `Read Message History` permission so that I can run properly. See below.\n"
            });
            return message.channel.send({
                content: "https://cdn.discordapp.com/attachments/688229874649137188/865342658242019368/Screenshot_20210715_172209.png"
            });
        }
        ...
        if (message.content.startsWith(prefix)) {
            ...
            if (timestamps.has(message.author.id)) {
                ...
                try {
                    ...
                    console.error(err);
                    message.reply({
                        content: "Unfortunately, there was an error upon executing this command"
                    });
                }
            }
        }

    ...
    run: async (client, message, args) => {
        ...
        if (args == `${PREFIX}lyrics`) {
            ...
            api.other.lyrics(args).then(res => {
                ...
                if (!message.channel.nsfw) {
                    for (let i = 0; i < bannedWords.length; i++) {
                        if (res.lyrics.includes(bannedWords[i])) return message.channel.send("This song contains NSFW lyrics. Please visit an NSFW channel to view this song's lyrics.");
                    }

                    for (let j = 0; j < bannedPhrases.length; j++) {
                        if (res.lyrics.includes(bannedPhrases[j])) return message.channel.send("This song contains NSFW lyrics. Please visit an NSFW channel to view this song's lyrics.");
                    }
                }
                let lyricsEmbed = {

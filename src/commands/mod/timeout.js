    ...
    callback: async (client, interaction) => {
        ...
        try {
            ...
            if (targetUser.isCommunicationDisabled()) {
                await targetUser.timeout(msDuration, reason ? reason : "No reason provided.");
                await interaction.editReply(`${targetUser}'s timeout has been updated to ${prettyMs(msDuration, { verbose: true })} ${reason ? `for **${reason}**` : ""}`);
                return;
            }

            await targetUser.timeout(msDuration, reason ? reason : "No reason provided");
            await interaction.editReply(`${targetUser} was timed out for ${prettyMs(msDuration, { verbose: true })} ${reason ? `for **${reason}**` : ""}`);
        } catch (error) {

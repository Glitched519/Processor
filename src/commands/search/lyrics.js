        ...
        axios.request(options).then(response => {
            ...
            if (!message.channel.nsfw) {
                for (let i = 0; i < bannedWords.length; i++) {
                    if (res.lyrics == bannedWords[i]) return message.reply({ content: `${res.title} contains NSFW lyrics (${bannedWords[i]}). Please visit an NSFW channel to view this song"s lyrics.` })
                }

                for (let j = 0; j < bannedPhrases.length; j++) {
                    if (res.lyrics.includes(bannedPhrases[j])) return message.reply({ content: `${res.title} contains NSFW lyrics (${bannedPhrases[j]}). Please visit an NSFW channel to view this song"s lyrics.` })
                }
            }
